module PrimalDualInteriorPoint (runSteps, dualGap) where
...
gradMeritFunc x rho nu = c + scalar rho * (tr a) #> (hPrimal x) / scalar (norm_2 $ hPrimal x) + scalar nu * (cmap (\xi -> -1/xi) x) 
...
hDual :: SubState -> SubState -> Vector Double
hDual y s = tr a #> y + s - c
...
runSteps state_k rho nu count path
    ...
    where
        ...
        dual_constraint_error   = norm_2 $ hDual   yk sk
